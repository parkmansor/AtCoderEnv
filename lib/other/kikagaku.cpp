...
D distSP(P a1, P a2, P p) {
  P r = proj(a1, a2, p);
  if (isecSP(a1, a2, r)) return abs(r - p);	// 線分に射影点がある
  return min(abs(a1 - p), abs(a2 - p));		// 線分に射影点がない
}
...
int main()
{
	...
	REP(i, N) {
		cin >> x >> y;
		starListA.emplace_back(x, y);
	}
	REP(i, N) {
		cin >> x >> y;
		starListB.emplace_back(x, y);
	}

	// 凸包
	vector<P> hullA = convexHull(starListA);
	vector<P> hullB = convexHull(starListB);
	
	// 倍率 
	D distA;
	D distB;
	for (int i = 0; i < hullA.size(); i++) {
		// A[0]→A[1] vs B[i]→B[i+1]の倍率
		distA = abs(hullA[1] - hullA[0]);
		if (i == hullA.size() - 1) {
			distB = abs(hullB[i] - hullB[0]);
		} else {
			distB = abs(hullB[i + 1] - hullB[i]);
		}
		D rate = distB / distA;

		// 全部倍率が同じはず
		bool ans = true;
		for (int idxA = 0; idxA < hullA.size(); idxA++) {
			int idxB = (i + idxA) % hullA.size();
			if (idxA == hullA.size() - 1) {
				distA = abs(hullA[idxA] - hullA[0]);
			} else {
				distA = abs(hullA[idxA + 1] - hullA[idxA]);
			}
			if (idxB == hullA.size() - 1) {
				distB = abs(hullB[idxB] - hullB[0]);
			} else {
				distB = abs(hullB[idxB + 1] - hullB[idxB]);
			}
			if (!EQ(rate, distB / distA)) {
				ans = false;
				break;
			}
		}
